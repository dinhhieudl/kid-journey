 ...
 * Thêm các bảng mới cho Phase 1 (Auth) và Phase 2 (Growth WHO, Montessori Tracking)
 * 
 * Chạy: node scripts/migrate.js
 * An toàn: chỉ thêm bảng mới, không xóa/sửa bảng cũ
 */
...
const dbPath = path.join(__dirname, '..', 'data', 'kidjourney.db');
...
if (fs.existsSync(whoDataPath)) {
  ...
  console.log('\n⚠️  who-lms-data.json not found. Run: node scripts/prepare-who-data.js');
}
